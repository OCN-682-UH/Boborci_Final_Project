...
```{r}
#| code-fold: true
tile_data<-read.csv(here("Chagos_Tile_App","annotations.csv"))
#View(tile_data)
...
write_csv(here("Chagos_Tile_App","Data","clean_tile_data.csv"))
```

Make another dataset that's a filtered subset for easier viewing:
```{r}
#| code-fold: true
tile_data_small<-clean_tile_data%>%
  filter(Label.code %in% c("_Co","_RDUP","TURF","_CAUL","_LOBO", "_CCA", "GREEN", "_CCAG", "_GREN","_SED"))%>%
write_csv(here("Chagos_Tile_App","Data","tile_data_small.csv"))
```
...
Create markdown friendly table for Readme file
```{r}
#| code-fold: TRUE
data_frame <- read.csv(here("Chagos_Tile_App","Data_Dictionary.csv")) #name dataframe to use


markdown_table_text <- kable(data_frame, format = "markdown",row.names=FALSE)   #use kable to make a table, format markdown

writeLines(markdown_table_text, "temp_readme_table.txt")   #this preserves the whole table, had issues earlier with it truncating so had to save like this 
```
